<script lang="ts">
    ...
    let searchQueries = $state<Record<string, string>>(Object.keys(facets).reduce<Record<string, string>>((acc, key) => {
        acc[key] = '';
        return acc;
    }, {}));

    let showAll = $state<Record<string, boolean>>(Object.keys(facets).reduce<Record<string, boolean>>((acc, key) => {
        acc[key] = false;
        return acc;
    }, {}));
            ...
            <ul class="dropdown-content menu bg-base-100 rounded-box w-full p-2 shadow">
                <li>
                    <input
                        ...
                        oninput={(e) => updateSearchQuery(facetKey, (e.target as HTMLInputElement)?.value ?? '')}
                    />
                </li>
                {#each getVisibleOptions(facet.items, searchQueries[facetKey], facetKey) as item}
                    <li>
                        <label class="flex items-center">
                            <input type="checkbox" class="checkbox checkbox-secondary" bind:checked={item.selected} />
