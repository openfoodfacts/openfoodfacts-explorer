<script lang="ts">
	...
	let currentLang = $derived(product.lang || 'en');
	let packagingTextField = $derived(`packaging_text_${currentLang}` as keyof Product);
	...
	if (!product.packagings || product.packagings.length === 0) {
		product.packagings = [createEmptyComponent()];
	}
	...
	function addComponent() {
		product = {
			...product,
			packagings: [...(product.packagings || []), createEmptyComponent()]
		};
	}
	...
	async function fetchSuggestions(
		...
		try {
			const suggestions = await getPackagingTaxonomySuggestions(
				fetch,
				tagtype,
	...
	let packagingsComplete = $state(product.packagings_complete === 1);

	$effect(() => {
		product = { ...product, packagings_complete: packagingsComplete ? 1 : 0 };
	});
</script>
...
<div class="bg-base-100 border-base-300 mb-6 rounded-lg border p-4 shadow-sm">
	<div class="form-control w-full">
		...
		<textarea
			...
			bind:value={product[packagingTextField]}
		></textarea>
	...
	<div class="form-control mt-2">
		<label class="label cursor-pointer justify-start gap-3">
			<input
				...
				bind:checked={packagingsComplete}
			/>
