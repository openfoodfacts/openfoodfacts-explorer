export type Gs1Allocation = { name: string; prefixes: number[]; code: string };

const gs1Allocations: Gs1Allocation[] = [
...
export function getGs1Allocation(barcode: string): Gs1Allocation {
	for (const digits of [3, 2, 1]) {
		const prefix = Number.parseInt(barcode.slice(0, digits));
		const allocation = gs1Allocations.find((item) => item.prefixes.includes(prefix));
